...
#define POS2ID(a,b,c,d,e,f) ( (a)*(d)*(e) + (b)*(d) + (c))
...
int MallocVoxel(VOXEL *v,double *b,UINT *s,double step){
 ...
 //class
 if((v->cl=(short *)malloc(sizeof(short)*size))==NULL)
  return FALSE;

 //corner 01
 if((v->corner=(unsigned short *)malloc(sizeof(unsigned short)*size2))==NULL)
  return FALSE;
 //corner value calloc
...
int SetupVoxelTbl(unsigned short *tbl,unsigned int *Nt,VOXEL *v){
 UINT cnt=0;
 UINT i1,i2,i3;
 UINT id,tmp=0;
 Nt[0]=0;
  ...
  for(i2=0;i2<v->size[1];i2++){
   for(i3=0;i3<v->size[2];i3++){
    ...
    if(v->cl[id] !=0 && v->cl[id] !=255){
     ...
     tmp=Nt[cnt+1];
     //printf("class=%d\n",NrNumberOfVertexOnClass[v->cl[id]]);
     cnt++;
    }
   }
  }
...
int SetupVertexValue(unsigned short *t,unsigned int  n,VOXEL *v){
 UINT i1,i2,i3;
...
int RenderTo3D(unsigned short *t,unsigned int *Nt,unsigned int  n,VOXEL *v){
 UINT i1,i2,i3;
 int id,class,cid1,cid2;
 int p1,p2;
 double shift;
...
int RenderToPly(unsigned int r,unsigned short *t,unsigned int *Nt,unsigned int  n,VOXEL *v,PLY *ply){
 UINT i1,i2,i3;
 int id,class,cid1,cid2;
 int p1,p2;
 double shift;
...
int ShowPlyFormat(unsigned short *t,unsigned int *Nt,unsigned int  n,VOXEL *v){
 UINT i1,i2,i3;
 int id,class,cid1,cid2;
 int p1,p2;
 double shift;
...
int OutCAMtx(PLY *ply,PDB *p){
 int i1,i2;
 puts("#order1\treal1\torder2\treal2\tarea");
 for(i1=0;i1<ply->Nr;i1++){
  for(i2=i1+1;i2<ply->Nr;i2++){
   if(ply->area[i1][i2]==0||ply->area[i2][i1]==0)
   continue;
   printf("%d\t%d\t%d\t%d\t%.3f\n",i1,p->ResNum[i1],i2,p->ResNum[i2],ply->area[i1][i2]);
   printf("%d\t%d\t%d\t%d\t%.3f\n",i2,p->ResNum[i2],i1,p->ResNum[i1],ply->area[i2][i1]);
  }
  //exposed area
  printf("%d\t%d\t%d\t%d\t%.3f\n",i1,p->ResNum[i1],-1,-1,ply->area[i1][ply->Nr]);//exposed
 }
 return 0;
}
