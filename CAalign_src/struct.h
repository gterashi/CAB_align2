...
typedef struct{
	...
	char d1[LIN]; char d2[LIN];//dmtx file name
	char ddi[LIN];
	int ref;
	//ShibyaA1
	char q[LIN];
	...
	int mkdb_mode;
	double rms,rate;
	bool jmode,fmode,IgBroken,A3mode,NVmode,LQmode;

	//HPMC
	double grid,dif,ang,std;
	double g_step;
	bool MtxMode,PlyMode;
	double MetaC;
	//Dali
	bool IgNeighbor,CrossOver;
	double zmc,area;
	double LocalRate,ExtRate;	
...
static char AACODE[20]="AVFPMILDEKRSTYHCNWQG";
...
#define LEFT 1
#define DIA 2
#define STOP 3 //No more alignment from here
#define GAP -1
...
typedef struct{
	...
	int N,Max;
	double Tarea,TareaSq,Tarea4;//Total Area
}DMTX;
