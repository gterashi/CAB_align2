...
*/
    mov_com[0] = 0.0; mov_com[1] = 0.0; mov_com[2] = 0.0;
    ref_com[0] = 0.0; ref_com[1] = 0.0; ref_com[2] = 0.0;
  ...
  for (n=0; n<n_list; n++){
    ...
    //}
   mov_com[0] += mov_xlist[n][0];
   mov_com[1] += mov_xlist[n][1];
   mov_com[2] += mov_xlist[n][2];

   ref_com[0] += ref_xlist[n][0];
   ref_com[1] += ref_xlist[n][1];
   ref_com[2] += ref_xlist[n][2];
  ...
  for (n=0; n<n_list; n++){ 
    ...
    //}
   mov_xlist[n][0] -= mov_com[0];
   mov_xlist[n][1] -= mov_com[1];
   mov_xlist[n][2] -= mov_com[2];

   ref_xlist[n][0] -= ref_com[0];
   ref_xlist[n][1] -= ref_com[1];
   ref_xlist[n][2] -= ref_com[2];
  }
  ...
  *E0 = 0.0;
  ...
  for (n=0; n<n_list; n++) 
  {
    ...
    //}
        R[0][0] += mov_xlist[n][0] * ref_xlist[n][0];
        R[0][1] += mov_xlist[n][0] * ref_xlist[n][1];
        R[0][2] += mov_xlist[n][0] * ref_xlist[n][2];
        R[1][0] += mov_xlist[n][1] * ref_xlist[n][0];
        R[1][1] += mov_xlist[n][1] * ref_xlist[n][1];
        R[1][2] += mov_xlist[n][1] * ref_xlist[n][2];
        R[2][0] += mov_xlist[n][2] * ref_xlist[n][0];
        R[2][1] += mov_xlist[n][2] * ref_xlist[n][1];
        R[2][2] += mov_xlist[n][2] * ref_xlist[n][2];
	
  }
  *E0 *= 0.5;
}

void setup_rotation_noshift(double ref_xlist[][3],
                    double mov_xlist[][3], 
                    int n_list,
                    double mov_com[3],
                    double mov_to_ref[3],
...
//      *E0 +=  mov_xlist[n][i] * mov_xlist[n][i]  
//            + ref_xlist[n][i] * ref_xlist[n][i];
      *E0 +=  mov_xlist[n][0] * mov_xlist[n][0]+ ref_xlist[n][0] * ref_xlist[n][0];
      *E0 +=  mov_xlist[n][1] * mov_xlist[n][1]+ ref_xlist[n][1] * ref_xlist[n][1];
      *E0 +=  mov_xlist[n][2] * mov_xlist[n][2]+ ref_xlist[n][2] * ref_xlist[n][2];
    /*
     * correlation matrix R:   
     *   R[i,j) = sum(over n): y(n,i) * x(n,j)  
     *   where x(n) and y(n) are two vector sets   
                             ...
                             double* rmsd)
{
 ...
 void calculate_rotation_rmsd_noshift(double ref_xlist[][3],
                             double mov_xlist[][3], 
                             int n_list,
                             double mov_com[3],
                             double mov_to_ref[3],
                             ...
                             double* rmsd)
{
  ...
  setup_rotation_noshift(ref_xlist, mov_xlist, n_list, 
                 mov_com, mov_to_ref, R, &Eo);
  calculate_rotation_matrix(R, U, Eo, &residual);
...
//if residual is too low, *rms=nan
...
void fast_rmsd_noshift(double ref_xlist[][3],
               double mov_xlist[][3], 
               int n_list,
               double* rmsd)
{ 
