...
int LowerBonds(char **list,int N,int len,int LenOfSeg,FTABLE *ft,int *res_list){
 ...
 //New check Missing residues
 //Miss=(double *)calloc(RES,sizeof(double));
  ...
  num=0;
	...
	*/
	for(pos=0;pos<=num-2*len;pos++){
	 ...
	 //
	 //ft[start_tot+pos].f=(int)50*sqrt((X(gx[pos]-gx[pos+len])
	 //+X(gy[pos]-gy[pos+len]) +X(gz[pos]-gz[pos+len]))/(len*len));
	 //more simple
	...
	*/
	...
	for(pos=0;pos<=num-3*len;pos++){
	 //ft[start_tot+pos].f15[1]=(int)50*sqrt((X(gx[pos]-gx[pos+2*len])
	 //+X(gy[pos]-gy[pos+2*len]) +X(gz[pos]-gz[pos+2*len]))/(len*len));
