...
int MallocPdb(PDB *p,int n){
 int i;
	...
	if((p->AtomOnRes=(int *)malloc(sizeof(int)*n))==NULL){
	 return -1;
	}
	...
	//Chain ID
	if((p->Cid=(int *)malloc(sizeof(int)*n))==NULL){
	 return -1;
	}
 //puts("#fin malloc");
 return 0;
...
void ChkMissRes(PDB *pdb,bool *tbl){
 int i,j;
 for(i=0;i<pdb->NumOfRes;i++)
  tbl[i]=true;

 for(i=0;i<pdb->NumOfAtom;i++){
  j=pdb->AtomOnRes[i];
  if(!strncmp(pdb->TypeAtom[i],"CA ",3)){
   //printf("Find CA in %d res= %d\n",i,j);
   tbl[j]=false;
  }
 }
}

void ShiftMissRes(PDB *pdb,bool *tbl){
 int i,j;
 int Na,Nr;
 bool flag=false;
 Na=Nr=0;
 ...
 for(i=0;i<pdb->NumOfAtom;i++){
  ...
  //Shift atom data
  pdb->AtomOnRes[Na]=pdb->AtomOnRes[i];//Order base
  pdb->Chain[Na]=pdb->Chain[i];
  pdb->AtomNum[Na]=pdb->AtomNum[i];
  strcpy(pdb->TypeAtom[Na],pdb->TypeAtom[i]); 
  pdb->TypeAtomId[Na]=pdb->TypeAtomId[i];			

  pdb->xyz[Na][0]=pdb->xyz[i][0];
  pdb->xyz[Na][1]=pdb->xyz[i][1];
  pdb->xyz[Na][2]=pdb->xyz[i][2];
 ...
 for(i=0;i<pdb->NumOfRes;i++){
  ...
  pdb->ResNum[Nr]=pdb->ResNum[j];//Real number
  //Residue Type
  strcpy(pdb->TypeRes[Nr],pdb->TypeRes[j]);
  pdb->TypeResId[Nr]=pdb->TypeResId[j];
  pdb->ResOnAtom[Nr]=pdb->ResOnAtom[j];
  pdb->Cid[Nr]=pdb->Cid[j];
  Nr++;
 }
 if(flag==true)
  pdb->NumOfRes=Nr;
